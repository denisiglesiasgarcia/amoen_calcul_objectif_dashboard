...
from sections.helpers.amoen_historique import (
    create_barplot_historique_amoen
)
...
def load_projets_liste(project_name):
    nom_projets_liste = mycol_historique_sites.distinct('nom_projet', {'amoen_id': username})
...
config = mycol_authentification_site.find_one({'_id': ObjectId('66ad0b102f5cc6cb3c64e1d1')})
...
username, authentication_status, username_login = authenticator.login(location="sidebar")

if st.session_state['authentication_status']:
    # st.write(config['credentials']['usernames'][st.session_state['username']]['password'])
    with st.sidebar.popover('Change password'):
        try:
            if authenticator.reset_password(username=st.session_state['username'],
                                                fields={'Form name':'Remplacer mot de passe',
                ...
                mycol_authentification_site.update_one({'_id': ObjectId('66ad0b102f5cc6cb3c64e1d1')}, {"$set": config})
    ...
    tab1, tab2, tab3, tab4, tab5, tab6= st.tabs(["0 Readme", '1 Données site', "2 Note de calcul", '3 Résultats', '4 Historique', '5 Générer Rapport'])
    ...
    with tab1:
        ...
        st.write("Le fait que les unités soient des MJ/m² pour les deux peux porter à confusion. Il est important de comprendre leurs spécificités.")
    ...
    with tab2:
        ...
        def tab2_fragment():
            ...
            with tab2_col01:
                ...
                # SRE rénovée
                sre_renovation_m2 = st.text_input("SRE rénovée (m²):", value=data_sites_db['sre_renovation_m2'], help="La SRE rénovée est la partie du batiment qui a été rénovée, la surélévation/extension n'est pas incluse")
                validate_input("SRE rénovée:", sre_renovation_m2, "m²")
                st.session_state['data_site']['sre_renovation_m2'] = float(sre_renovation_m2)
            with tab2_col02:
                st.session_state['data_site']['adresse_projet'] = st.text_input("Adresse(s) du projet", value=data_sites_db['adresse_projet'])
                st.session_state['data_site']['amoen_id'] = st.text_input("AMOen", value=data_sites_db['amoen_id'])

            try:
                if st.session_state['data_site']['sre_renovation_m2'] <= 0:
                    st.warning(f"SRE doit être > 0 ({st.session_state['data_site']['sre_renovation_m2']})")
            except ValueError:
                st.warning("Problème dans la somme des pourcentages des affectations")

            st.markdown('<span style="font-size:1.2em;">**IDC moyen avant travaux et objectif en énergie finale**</span>', unsafe_allow_html=True)
            ...
            st.session_state['data_site']['delta_ef_visee_kwh_m2'] = st.session_state['data_site']['ef_avant_corr_kwh_m2'] -\
                                                                    st.session_state['data_site']['ef_objectif_pondere_kwh_m2']

            st.markdown('<span style="font-size:1.2em;">**Répartition énergie finale ECS/Chauffage**</span>', unsafe_allow_html=True)
            ...
            with tab2_col7:
                ...
                repartition_energie_finale_partie_renovee_chauffage = st.text_input("Chauffage partie rénovée [%]",
                                                                                    value=data_sites_db['repartition_energie_finale_partie_renovee_chauffage'],
            ...
            with tab2_col8:
                repartition_energie_finale_partie_renovee_ecs = st.text_input("ECS partie rénovée [%]",
                                                                            value=data_sites_db['repartition_energie_finale_partie_renovee_ecs'],
            ...
            try:
                repartition_ef_somme_avertissement = st.session_state['data_site']['repartition_energie_finale_partie_renovee_chauffage'] + \
                    st.session_state['data_site']['repartition_energie_finale_partie_renovee_ecs'] +\
                    st.session_state['data_site']['repartition_energie_finale_partie_surelevee_chauffage'] + \
                    st.session_state['data_site']['repartition_energie_finale_partie_surelevee_ecs']
                if repartition_ef_somme_avertissement != 100:
                    st.warning(f"La somme des pourcentages de répartition de l'énergie finale doit être égale à 100% ({repartition_ef_somme_avertissement}%)")
            except ValueError:
                st.warning("Problème dans la somme des pourcentages de répartition de l'énergie finale")


            st.subheader('Eléments à renseigner', divider='rainbow')

            st.markdown('<span style="font-size:1.2em;">**Sélectionner les dates de début et fin de période**</span>', unsafe_allow_html=True)
            ...
            try:
                if st.session_state['data_site']['periode_nb_jours'] <= 180:
                    st.warning("La période de mesure doit être supérieure à 3 mois (minimum recommandé 6 mois)")
            except ValueError:  
                st.warning("Problème de date de début et de fin de période")
            st.write(f"Période du {st.session_state['data_site']['periode_start'].strftime('%Y-%m-%d')} au {st.session_state['data_site']['periode_end'].strftime('%Y-%m-%d')} soit {int(st.session_state['data_site']['periode_nb_jours'])} jours")

            tab2_col1, tab2_col2 = st.columns(2)
            with tab2_col1:
                # SRE pourcentage
                st.markdown('<span style="font-size:1.2em;">**Affectations**</span>', unsafe_allow_html=True)
                ...
                if data_sites_db:
                    for option in options_sre_pourcentage:
                        if option['label'] in selected_sre_pourcentage:
                            value = st.text_input(option['label'] + ':', value=data_sites_db[option['variable']])
                            if value != "0":
                ...
                st.session_state['data_site']['sre_pourcentage_habitat_collectif'] = options_sre_pourcentage[0]['value']
                st.session_state['data_site']['sre_pourcentage_habitat_individuel'] = options_sre_pourcentage[1]['value']
                st.session_state['data_site']['sre_pourcentage_administration'] = options_sre_pourcentage[2]['value']
                st.session_state['data_site']['sre_pourcentage_ecoles'] = options_sre_pourcentage[3]['value']
                st.session_state['data_site']['sre_pourcentage_commerce'] = options_sre_pourcentage[4]['value']
                st.session_state['data_site']['sre_pourcentage_restauration'] = options_sre_pourcentage[5]['value']
                st.session_state['data_site']['sre_pourcentage_lieux_de_rassemblement'] = options_sre_pourcentage[6]['value']
                st.session_state['data_site']['sre_pourcentage_hopitaux'] = options_sre_pourcentage[7]['value']
                st.session_state['data_site']['sre_pourcentage_industrie'] = options_sre_pourcentage[8]['value']
                st.session_state['data_site']['sre_pourcentage_depots'] = options_sre_pourcentage[9]['value']
                st.session_state['data_site']['sre_pourcentage_installations_sportives'] = options_sre_pourcentage[10]['value']
                st.session_state['data_site']['sre_pourcentage_piscines_couvertes'] = options_sre_pourcentage[11]['value']

                # Somme des pourcentages
                try:
                    sre_pourcentage_affectations_somme_avertisement = \
                        float(st.session_state['data_site']['sre_pourcentage_habitat_collectif']) + \
                        float(st.session_state['data_site']['sre_pourcentage_habitat_individuel']) + \
                        float(st.session_state['data_site']['sre_pourcentage_administration']) + \
                        float(st.session_state['data_site']['sre_pourcentage_ecoles']) + \
                        float(st.session_state['data_site']['sre_pourcentage_commerce']) + \
                        float(st.session_state['data_site']['sre_pourcentage_restauration']) + \
                        float(st.session_state['data_site']['sre_pourcentage_lieux_de_rassemblement']) + \
                        float(st.session_state['data_site']['sre_pourcentage_hopitaux']) + \
                        float(st.session_state['data_site']['sre_pourcentage_industrie']) + \
                        float(st.session_state['data_site']['sre_pourcentage_depots']) + \
                        float(st.session_state['data_site']['sre_pourcentage_installations_sportives']) + \
                        float(st.session_state['data_site']['sre_pourcentage_piscines_couvertes'])
                    if sre_pourcentage_affectations_somme_avertisement != 100:
                        st.warning(f"Somme des pourcentages doit être égale à 100% ({sre_pourcentage_affectations_somme_avertisement})")
                except ValueError:
                    st.warning("Problème dans la somme des pourcentages des affectations")

            with tab2_col2:
                # Agents énergétiques
                st.markdown('<span style="font-size:1.2em;">**Agents énergétiques utilisés**</span>', unsafe_allow_html=True)
                ...
                if st.session_state['data_site']['nom_projet'] == 'Avusy 10-10A':
                    conso_elec_pac_immeuble, nearest_start_date, nearest_end_date = avusy_consommation_energie_elec_periode(st.session_state['data_site']['periode_start'], st.session_state['data_site']['periode_end'], mycol_historique_index_avusy)
                    if conso_elec_pac_immeuble and nearest_start_date.date() ==  st.session_state['data_site']['periode_start'].date() and nearest_end_date.date() == st.session_state['data_site']['periode_end'].date():
                        success = st.success("Dates OK!")
                        time.sleep(3)
                        success.empty()
                    else:
                        st.warning(f"Pas de données pour ces dates, dates les plus proches: du {nearest_start_date.date()} au {nearest_end_date.date()}")
                ...
                if st.session_state['data_site']['nom_projet'] != 'Avusy 10-10A':
                    for option in options_agent_energetique_ef:
                        if option['label'] in selected_agent_energetique_ef:
                            value = st.text_input(option['label'] + ':', value=0.0)
                            if value != "0":
                                value = validate_agent_energetique_input(option['label'] + ":", value, option['unit'])
                                option['value'] = float(value)
                    
                    st.session_state['data_site']['agent_energetique_ef_mazout_kg'] = options_agent_energetique_ef[6]['value']
                    st.session_state['data_site']['agent_energetique_ef_mazout_litres'] = options_agent_energetique_ef[5]['value']
                    st.session_state['data_site']['agent_energetique_ef_mazout_kwh'] = options_agent_energetique_ef[7]['value']
                    st.session_state['data_site']['agent_energetique_ef_gaz_naturel_m3'] = options_agent_energetique_ef[4]['value']
                    st.session_state['data_site']['agent_energetique_ef_gaz_naturel_kwh'] = options_agent_energetique_ef[3]['value']
                    st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_stere'] = options_agent_energetique_ef[8]['value']
                    st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_stere'] = options_agent_energetique_ef[9]['value']
                    st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_kwh'] = options_agent_energetique_ef[10]['value']
                    st.session_state['data_site']['agent_energetique_ef_pellets_m3'] = options_agent_energetique_ef[11]['value']
                    st.session_state['data_site']['agent_energetique_ef_pellets_kg'] = options_agent_energetique_ef[12]['value']
                    st.session_state['data_site']['agent_energetique_ef_pellets_kwh'] = options_agent_energetique_ef[13]['value']
                    st.session_state['data_site']['agent_energetique_ef_plaquettes_m3'] = options_agent_energetique_ef[14]['value']
                    st.session_state['data_site']['agent_energetique_ef_plaquettes_kwh'] = options_agent_energetique_ef[15]['value']
                    st.session_state['data_site']['agent_energetique_ef_cad_kwh'] = options_agent_energetique_ef[0]['value']
                    st.session_state['data_site']['agent_energetique_ef_electricite_pac_kwh'] = options_agent_energetique_ef[1]['value']
                    st.session_state['data_site']['agent_energetique_ef_electricite_directe_kwh'] = options_agent_energetique_ef[2]['value']
                    st.session_state['data_site']['agent_energetique_ef_autre_kwh'] = options_agent_energetique_ef[16]['value']
                
                elif st.session_state['data_site']['nom_projet'] == 'Avusy 10-10A':
                    for option in options_agent_energetique_ef:
                        if option['label'] in selected_agent_energetique_ef:
                            if option['label'] == 'Electricité pour les PAC (kWh)':
                                value = st.text_input(option['label'] + ':', value=round(conso_elec_pac_immeuble, 1))
                                if value != "0":
                                    value = validate_agent_energetique_input(option['label'] + ":", value, option['unit'])
                                    option['value'] = float(value)
                            else:
                ...
                try:
                    agent_energetique_ef_somme_avertissement = \
                        float(st.session_state['data_site']['agent_energetique_ef_mazout_kg']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_mazout_litres']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_mazout_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_gaz_naturel_m3']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_gaz_naturel_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_stere']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_stere']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_pellets_m3']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_pellets_kg']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_pellets_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_plaquettes_m3']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_plaquettes_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_cad_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_electricite_pac_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_electricite_directe_kwh']) + \
                        float(st.session_state['data_site']['agent_energetique_ef_autre_kwh'])
                    if agent_energetique_ef_somme_avertissement <= 0:
                        st.warning(f"Veuillez renseigner une quantité d'énergie utilisée sur la période ({agent_energetique_ef_somme_avertissement})")
                except ValueError:
        ...
        if st.session_state['data_site']['nom_projet'] == 'Avusy 10-10A':
            ...
            avusy_consommation_energie_dashboard(st.session_state['data_site']['periode_start'], st.session_state['data_site']['periode_end'], mycol_historique_index_avusy)
    ...
    with tab3:
        ...
        # C65 → Début période
        df_periode_list.append({'Dénomination': 'Début période', 'Valeur': st.session_state['data_site']['periode_start'], 'Unité': '-', 'Commentaire': 'Date de début de la période', 'Excel': 'C65', 'Variable/Formule' : 'periode_start'})
        
        # C66 → Fin période
        df_periode_list.append({'Dénomination': 'Fin période', 'Valeur': st.session_state['data_site']['periode_end'], 'Unité': '-', 'Commentaire': 'Date de fin de la période', 'Excel': 'C66', 'Variable/Formule': 'periode_end'})

        # C67 → Nombre de jours
        df_list.append({'Dénomination': 'Nombre de jour(s)', 'Valeur': st.session_state['data_site']['periode_nb_jours'], 'Unité': 'jour(s)', 'Commentaire': 'Nombre de jour(s) dans la période', 'Excel': 'C67', 'Variable/Formule': 'periode_nb_jours'})

        # C86 → Répartition en énergie finale - Chauffage partie rénovée
        df_list.append({'Dénomination': 'Répartition en énergie finale (chauffage) pour la partie rénové','Valeur': st.session_state['data_site']['repartition_energie_finale_partie_renovee_chauffage'], 'Unité': '%',
            'Commentaire': '', 'Excel': 'C86', 'Variable/Formule': "repartition_energie_finale_partie_renovee_chauffage"
        })

        # C87 → Répartition en énergie finale - ECS partie rénovée
        df_list.append({
            'Dénomination': 'Répartition en énergie finale (ECS) pour la partie rénové',
            'Valeur': st.session_state['data_site']['repartition_energie_finale_partie_renovee_ecs'],
            'Unité': '%',
        ...
        df_list.append({
            ...
            'Valeur': st.session_state['data_site']['repartition_energie_finale_partie_surelevee_chauffage'],
            'Unité': '%',
        ...
        df_list.append({
            ...
            'Valeur': st.session_state['data_site']['repartition_energie_finale_partie_surelevee_ecs'],
            'Unité': '%',
        ...
        st.session_state['data_site']['repartition_energie_finale_partie_renovee_somme'] = st.session_state['data_site']['repartition_energie_finale_partie_renovee_chauffage'] + st.session_state['data_site']['repartition_energie_finale_partie_renovee_ecs']
        df_list.append({
            'Dénomination': 'Part EF pour partie rénové (Chauffage + ECS)',
            'Valeur': st.session_state['data_site']['repartition_energie_finale_partie_renovee_somme'],
            'Unité': '%',
        ...
        st.session_state['data_site']['estimation_ecs_annuel'] = st.session_state['data_site']['periode_nb_jours']/365
        df_list.append({
        ...
        st.session_state['data_site']['dj_periode'] = float(calcul_dj_periode(df_meteo_tre200d0, st.session_state['data_site']['periode_start'], st.session_state['data_site']['periode_end']))
        st.session_state['data_site']['estimation_part_chauffage_periode_sur_annuel'] = float(st.session_state['data_site']['dj_periode'] / DJ_REF_ANNUELS)
        df_list.append({
            'Dénomination': 'Est. Chauffage/Chauffage annuel prévisible',
            'Valeur': st.session_state['data_site']['estimation_part_chauffage_periode_sur_annuel']*100,
            'Unité': '%',
        ...
        st.session_state['data_site']['estimation_energie_finale_periode_sur_annuel'] = (st.session_state['data_site']['estimation_ecs_annuel'] * (st.session_state['data_site']['repartition_energie_finale_partie_renovee_ecs'] + st.session_state['data_site']['repartition_energie_finale_partie_surelevee_ecs'])) +\
                                            (st.session_state['data_site']['estimation_part_chauffage_periode_sur_annuel'] * (st.session_state['data_site']['repartition_energie_finale_partie_renovee_chauffage'] + st.session_state['data_site']['repartition_energie_finale_partie_surelevee_chauffage']))
        df_list.append({
            'Dénomination': 'Est. EF période / EF année',
            'Valeur': st.session_state['data_site']['estimation_energie_finale_periode_sur_annuel'],
            'Unité': '%',
        ...
        try:
            if st.session_state['data_site']['estimation_energie_finale_periode_sur_annuel'] != 0:
                st.session_state['data_site']['part_ecs_periode_comptage'] = (st.session_state['data_site']['estimation_ecs_annuel'] * (st.session_state['data_site']['repartition_energie_finale_partie_renovee_ecs'] + \
                    st.session_state['data_site']['repartition_energie_finale_partie_surelevee_ecs'])) / st.session_state['data_site']['estimation_energie_finale_periode_sur_annuel']
            else:
        ...
        df_list.append({
            ...
            'Valeur': st.session_state['data_site']['part_ecs_periode_comptage']*100,
            'Unité': '%',
            'Commentaire': '',
            'Excel': 'C95',
        ...
        try:
            if st.session_state['data_site']['estimation_energie_finale_periode_sur_annuel'] != 0:
                st.session_state['data_site']['part_chauffage_periode_comptage'] = (st.session_state['data_site']['estimation_part_chauffage_periode_sur_annuel'] * \
                    (st.session_state['data_site']['repartition_energie_finale_partie_renovee_chauffage'] + st.session_state['data_site']['repartition_energie_finale_partie_surelevee_chauffage'])) / st.session_state['data_site']['estimation_energie_finale_periode_sur_annuel']
            else:
        ...
        df_list.append({
            ...
            'Valeur': st.session_state['data_site']['part_chauffage_periode_comptage']*100,
            'Unité': '%',
            'Commentaire': '',
            'Excel': 'C96',
        ...
        st.session_state['data_site']['correction_ecs'] = 365/st.session_state['data_site']['periode_nb_jours']
        df_list.append({
        ...
        st.session_state['data_site']['agent_energetique_ef_mazout_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_mazout_kg'] * CONVERSION_MAZOUT_KG_MJ + st.session_state['data_site']['agent_energetique_ef_mazout_litres'] * CONVERSION_MAZOUT_LITRES_MJ + st.session_state['data_site']['agent_energetique_ef_mazout_kwh'] * CONVERSION_MAZOUT_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_gaz_naturel_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_gaz_naturel_m3'] * CONVERSION_GAZ_NATUREL_M3_MJ + st.session_state['data_site']['agent_energetique_ef_gaz_naturel_kwh'] * CONVERSION_GAZ_NATUREL_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_stere'] * CONVERSION_BOIS_BUCHES_DUR_STERE_MJ)
        st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_stere'] * CONVERSION_BOIS_BUCHES_TENDRE_STERE_MJ + st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_kwh'] * CONVERSION_BOIS_BUCHES_TENDRE_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_pellets_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_pellets_m3'] * CONVERSION_PELLETS_M3_MJ + st.session_state['data_site']['agent_energetique_ef_pellets_kg'] * CONVERSION_PELLETS_KG_MJ + st.session_state['data_site']['agent_energetique_ef_pellets_kwh'] * CONVERSION_PELLETS_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_plaquettes_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_plaquettes_m3'] * CONVERSION_PLAQUETTES_M3_MJ + st.session_state['data_site']['agent_energetique_ef_plaquettes_kwh'] * CONVERSION_PLAQUETTES_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_cad_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_cad_kwh'] * CONVERSION_CAD_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_electricite_pac_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_electricite_pac_kwh'] * CONVERSION_ELECTRICITE_PAC_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_electricite_directe_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_electricite_directe_kwh'] * CONVERSION_ELECTRICITE_DIRECTE_KWH_MJ)
        st.session_state['data_site']['agent_energetique_ef_autre_somme_mj'] = (st.session_state['data_site']['agent_energetique_ef_autre_kwh'] * CONVERSION_AUTRE_KWH_MJ)

        st.session_state['data_site']['agent_energetique_ef_somme_kwh'] = (st.session_state['data_site']['agent_energetique_ef_mazout_somme_mj'] + \
                                                st.session_state['data_site']['agent_energetique_ef_gaz_naturel_somme_mj'] + \
                                                st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_somme_mj'] + \
                                                st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_somme_mj'] + \
                                                ...
                                                st.session_state['data_site']['agent_energetique_ef_electricite_pac_somme_mj'] + \
                                                st.session_state['data_site']['agent_energetique_ef_electricite_directe_somme_mj'] + \
                                                st.session_state['data_site']['agent_energetique_ef_autre_somme_mj']) / 3.6
        df_list.append({
            'Dénomination': 'Energie finale indiqué par le(s) compteur(s)',
            'Valeur': st.session_state['data_site']['agent_energetique_ef_somme_kwh'],
            'Unité': 'kWh',
        ...
        st.session_state['data_site']['methodo_b_ww_kwh'] = (st.session_state['data_site']['agent_energetique_ef_somme_kwh']) * st.session_state['data_site']['part_ecs_periode_comptage']
        df_list.append({
        ...
        # C100 → Methodo_Eww
        try:
            if st.session_state['data_site']['sre_renovation_m2'] != 0 and st.session_state['data_site']['periode_nb_jours'] != 0:
                st.session_state['data_site']['methodo_e_ww_kwh'] = (st.session_state['data_site']['methodo_b_ww_kwh'] / st.session_state['data_site']['sre_renovation_m2']) * (365 / st.session_state['data_site']['periode_nb_jours'])
            else:
        ...
        st.session_state['data_site']['methodo_b_h_kwh'] = st.session_state['data_site']['agent_energetique_ef_somme_kwh'] * st.session_state['data_site']['part_chauffage_periode_comptage']
        df_list.append({
        ...
        # C104 → Methodo_Eh
        try:
            if st.session_state['data_site']['sre_renovation_m2'] != 0 and st.session_state['data_site']['dj_periode'] != 0:
                st.session_state['data_site']['methodo_e_h_kwh'] = (st.session_state['data_site']['methodo_b_h_kwh'] / st.session_state['data_site']['sre_renovation_m2']) * (DJ_REF_ANNUELS / st.session_state['data_site']['dj_periode'])
            else:
        ...
        st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_inclus_surelevation_kwh_m2'] = st.session_state['data_site']['methodo_e_ww_kwh'] + st.session_state['data_site']['methodo_e_h_kwh']
        df_list.append({
            'Dénomination': 'Ef,après,corr (inclus surélévation)',
            'Valeur': st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_inclus_surelevation_kwh_m2'],
            'Unité': 'kWh/m²',
        ...
        st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_kwh_m2'] = st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_inclus_surelevation_kwh_m2'] * (st.session_state['data_site']['repartition_energie_finale_partie_renovee_somme'] / 100)
        df_list.append({
            'Dénomination': 'Ef,après,corr,rénové',
            'Valeur': st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_kwh_m2'],
            'Unité': 'kWh/m²',
        ...
        try:
            if st.session_state['data_site']['agent_energetique_ef_somme_kwh']:
                st.session_state['data_site']['facteur_ponderation_moyen'] = (st.session_state['data_site']['agent_energetique_ef_mazout_somme_mj'] * FACTEUR_PONDERATION_MAZOUT + \
                                    st.session_state['data_site']['agent_energetique_ef_gaz_naturel_somme_mj'] * FACTEUR_PONDERATION_GAZ_NATUREL + \
                                    st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_somme_mj'] * FACTEUR_PONDERATION_BOIS_BUCHES_DUR + \
                                    st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_somme_mj'] * FACTEUR_PONDERATION_BOIS_BUCHES_TENDRE + \
                                    st.session_state['data_site']['agent_energetique_ef_pellets_somme_mj'] * FACTEUR_PONDERATION_PELLETS + \
                                    st.session_state['data_site']['agent_energetique_ef_plaquettes_somme_mj'] * FACTEUR_PONDERATION_PLAQUETTES + \
                                    st.session_state['data_site']['agent_energetique_ef_cad_somme_mj'] * FACTEUR_PONDERATION_CAD + \
                                    st.session_state['data_site']['agent_energetique_ef_electricite_pac_somme_mj'] * FACTEUR_PONDERATION_ELECTRICITE_PAC + \
                                    st.session_state['data_site']['agent_energetique_ef_electricite_directe_somme_mj'] * FACTEUR_PONDERATION_ELECTRICITE_DIRECTE + \
                                    st.session_state['data_site']['agent_energetique_ef_autre_somme_mj'] * FACTEUR_PONDERATION_AUTRE) / (st.session_state['data_site']['agent_energetique_ef_somme_kwh'] * 3.6)
            else:
        ...
        # C109 → Methodo_Eww*fp
        st.session_state['data_site']['methodo_e_ww_renovee_pondere_kwh_m2'] = st.session_state['data_site']['methodo_e_ww_kwh'] * st.session_state['data_site']['facteur_ponderation_moyen'] * (st.session_state['data_site']['repartition_energie_finale_partie_renovee_somme'] / 100)
        df_list.append({
            'Dénomination': 'Methodo_Eww*fp',
            'Valeur': st.session_state['data_site']['methodo_e_ww_renovee_pondere_kwh_m2'],
            'Unité': 'kWh/m²',
            'Commentaire': '',
            'Excel': 'C109',
        ...
        # C110 → Methodo_Eh*fp
        st.session_state['data_site']['methodo_e_h_renovee_pondere_kwh_m2'] = st.session_state['data_site']['methodo_e_h_kwh'] * st.session_state['data_site']['facteur_ponderation_moyen'] * (st.session_state['data_site']['repartition_energie_finale_partie_renovee_somme'] / 100)
        df_list.append({
            'Dénomination': 'Methodo_Eh*fp',
            'Valeur': st.session_state['data_site']['methodo_e_h_renovee_pondere_kwh_m2'],
            'Unité': 'kWh/m²',
            'Commentaire': '',
            'Excel': 'C110',
        ...
        st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2'] = st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_kwh_m2'] * st.session_state['data_site']['facteur_ponderation_moyen']
        df_list.append({
            'Dénomination': 'Ef,après,corr,rénové*fp',
            'Valeur': st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2'],
            'Unité': 'kWh/m²',
            'Commentaire': '',
            'Excel': 'C113',
        ...
        st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_MJ_m2'] = st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2'] * 3.6
        df_list.append({
            'Dénomination': 'Ef,après,corr,rénové*fp',
            'Valeur': st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_MJ_m2'],
            'Unité': 'MJ/m²',
            'Commentaire': '',
            'Excel': 'C114',
        ...
        df_meteo_note_calcul = df_meteo_tre200d0[(df_meteo_tre200d0['time'] >= st.session_state['data_site']['periode_start']) & (df_meteo_tre200d0['time'] <= st.session_state['data_site']['periode_end'])][['time', 'tre200d0', 'DJ_theta0_16']]
        ...
        df_agent_energetique = pd.DataFrame({
            ...
            'Somme MJ' : [st.session_state['data_site']['agent_energetique_ef_mazout_somme_mj'], st.session_state['data_site']['agent_energetique_ef_gaz_naturel_somme_mj'], st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_somme_mj'], st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_somme_mj'], st.session_state['data_site']['agent_energetique_ef_pellets_somme_mj'], st.session_state['data_site']['agent_energetique_ef_plaquettes_somme_mj'], st.session_state['data_site']['agent_energetique_ef_cad_somme_mj'], st.session_state['data_site']['agent_energetique_ef_electricite_pac_somme_mj'], st.session_state['data_site']['agent_energetique_ef_electricite_directe_somme_mj'], st.session_state['data_site']['agent_energetique_ef_autre_somme_mj']],
            'Somme kWh' : [st.session_state['data_site']['agent_energetique_ef_mazout_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_gaz_naturel_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_pellets_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_plaquettes_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_cad_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_electricite_pac_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_electricite_directe_somme_mj'] / 3.6, st.session_state['data_site']['agent_energetique_ef_autre_somme_mj'] / 3.6],
        ...
        formula_facteur_ponderation_moyen = r"facteur\_ponderation\_moyen = \frac{{({0})}}{{({1})}} = {2}".format(
            st.session_state['data_site']['agent_energetique_ef_mazout_somme_mj'] * FACTEUR_PONDERATION_MAZOUT + \
            st.session_state['data_site']['agent_energetique_ef_gaz_naturel_somme_mj'] * FACTEUR_PONDERATION_GAZ_NATUREL + \
            st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_somme_mj'] * FACTEUR_PONDERATION_BOIS_BUCHES_DUR + \
            st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_somme_mj'] * FACTEUR_PONDERATION_BOIS_BUCHES_TENDRE + \
            st.session_state['data_site']['agent_energetique_ef_pellets_somme_mj'] * FACTEUR_PONDERATION_PELLETS + \
            ...
            st.session_state['data_site']['agent_energetique_ef_electricite_pac_somme_mj'] * FACTEUR_PONDERATION_ELECTRICITE_PAC + \
            st.session_state['data_site']['agent_energetique_ef_electricite_directe_somme_mj'] * FACTEUR_PONDERATION_ELECTRICITE_DIRECTE + \
            st.session_state['data_site']['agent_energetique_ef_autre_somme_mj'] * FACTEUR_PONDERATION_AUTRE,
    ...
    with tab4:
        ...
        # calculs
        st.session_state['data_site']['delta_ef_realisee_kwh_m2'] = st.session_state['data_site']['ef_avant_corr_kwh_m2'] - st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2']
        try:
            if st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2'] != 0:
                st.session_state['data_site']['atteinte_objectif'] = st.session_state['data_site']['delta_ef_realisee_kwh_m2']/st.session_state['data_site']['delta_ef_visee_kwh_m2']
            else:
        ...
        df_resultats1 = pd.DataFrame({
            ...
            'kWh/m²/an': [
                ...
                round(st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2'], 4),
            ...
            'MJ/m²/an': [
                round(st.session_state['data_site']['ef_avant_corr_kwh_m2']*3.6, 4),
                round(st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2']*3.6, 4),
                round(st.session_state['data_site']['ef_objectif_pondere_kwh_m2']*3.6, 4),
                round(st.session_state['data_site']['delta_ef_realisee_kwh_m2']*3.6, 4),
                round(st.session_state['data_site']['delta_ef_visee_kwh_m2']*3.6, 4)],
        })
        ...
        # Graphique 1
        if st.session_state['data_site']['amoen_id'] and \
            st.session_state['data_site']['ef_avant_corr_kwh_m2'] and \
            st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2'] and\
            st.session_state['data_site']['ef_objectif_pondere_kwh_m2']:
            
            graphique_bars_objectif_exploitation(st.session_state['data_site']['nom_projet'],
            ...
            st.warning("Veuillez compléter les informations dans '1 Données site' pour générer le graphique")
    ...
    with tab5:
        ...
        if selected_addresses:
            ...
            filtered_df = df_adresses_egid[df_adresses_egid['adresse'].isin(selected_addresses)]
        ...
        pipeline_historique_graphique = [
            {
                "$match": {
                    ...
                    "energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2": { "$ne": 0.0 }
                }
            },
            {
                "$group": {
        ...
        if data_db_historique:
            ...
            required_columns = ['nom_projet', 'periode_start', 'periode_end', 'atteinte_objectif']

            # Proceed only if all required columns are present
            if all([col in df_historique_complet.columns for col in required_columns]):
                fig_historique_amoen = create_barplot_historique_amoen(df_historique_complet)
    ...
    with tab6:
        ...
        if all([
            ...
            is_valid(st.session_state['data_site']['sre_pourcentage_habitat_collectif']),
            is_valid(st.session_state['data_site']['sre_pourcentage_habitat_individuel']),
            is_valid(st.session_state['data_site']['sre_pourcentage_administration']),
            ...
            is_valid(st.session_state['data_site']['sre_pourcentage_lieux_de_rassemblement']),
            ...
            is_valid(st.session_state['data_site']['sre_pourcentage_installations_sportives']),
            is_valid(st.session_state['data_site']['sre_pourcentage_piscines_couvertes']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_mazout_kg']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_mazout_litres']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_mazout_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_gaz_naturel_m3']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_gaz_naturel_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_bois_buches_dur_stere']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_stere']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_bois_buches_tendre_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_pellets_m3']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_pellets_kg']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_pellets_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_plaquettes_m3']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_plaquettes_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_cad_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_electricite_pac_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_electricite_directe_kwh']),
            is_valid(st.session_state['data_site']['agent_energetique_ef_autre_kwh']),
            ...
            is_valid(st.session_state['data_site']['energie_finale_apres_travaux_climatiquement_corrigee_renovee_pondere_kwh_m2']),
            ...
            is_valid(st.session_state['data_site']['repartition_energie_finale_partie_renovee_chauffage']),
            is_valid(st.session_state['data_site']['repartition_energie_finale_partie_renovee_ecs']),
            is_valid(st.session_state['data_site']['repartition_energie_finale_partie_surelevee_chauffage']),
            is_valid(st.session_state['data_site']['repartition_energie_finale_partie_surelevee_ecs']),
        ]):
            if st.button("Générer le rapport PDF"):
                ...
                st.success(f"Rapport PDF '{file_name}' généré avec succès! Cliquez sur le bouton ci-dessus pour le télécharger.")
